...
formulario.addEventListener("submit", (e)=>{
  ...
  //El objeto localStorage permite guardar pares clave/valor en el navegador sin fecha de caducidad, es decir, los datos no se borran cuando se cierra el navegador y 
  //quedan disponibles para futuras navegaciones.
...
campoDeFormulario.forEach((campo) => {
  ...
  campo.addEventListener("invalid", (evento) => evento.preventDefault); //Metodo preventDefault hace que cuando el campo sea invalido muestra un aviso
});
...
function verificarCampo(campo) {
  ...
  if (campo.name == "cuil" && campo.value.length >= 9) {
    esUnCuil(campo);
  }
